...
// newModelRepoDisabledServer stands in for the backend's response when Model
// Repo is disabled/inaccessible: a 200 with a top-level `errors` array and no
// data (see assertModelRepoFeatureEnabled in node/graphql/schema/modelRepo.ts).
...
// contract end to end for each `model` command: a Model Repo access failure
// must return a non-nil error (exit 1 via cmd/root.go), the command itself
// must stay silent (the sink owns printing), and the sink must emit a
// "graphql_error"-coded json object on stderr, not the generic "cli_error"
// fallback.
