...
var updateCmd = &cobra.Command{
	...
	Run: func(c *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Println("error fetching latest version info for runpod", err)
			return
		}
		...
		if runtime.GOOS == "windows" {
			...
			fmt.Printf("wget https://github.com/runpod/runpod/releases/download/%s/%s -O runpod.exe\n", latestVersion, newBinaryName)
		}
			fmt.Printf("downloading runpod %s to %s\n", latestVersion, downloadPath)
			file, err := DownloadFile(downloadLink, downloadPath)
			defer file.Close()
			if err != nil {
				fmt.Println("error fetching the latest version of runpod", err)
				return
			}
			//chmod +x
			err = file.Chmod(0755)
