...
- graphql `Pod.desiredStatus` and rest `/pods/{id}.desiredStatus` are separate snapshots and can disagree; anything derived from `runtime` must be gated on the desiredStatus from the *same* snapshot, or momentary skew silently defeats the gate (see `cmd/pod/get.go`).
