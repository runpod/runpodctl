...
- CON-683 exception to the above: errors must never reach stdout. `api/query.go` and `internal/api/graphql.go` return the typed `no_credentials` sentinel instead of printing; `cmd/model` and `cmd/exec` return every error to the `Execute` sink. still unconverted, and still violating this: `cmd/pod/*Pod.go`, `cmd/pods/*`, `cmd/cloud` (plaintext via `cobra.CheckErr`) and `cmd/project/*` (prints to stdout and exits 0 — was CON-816, now CON-874, which supersedes it and deletes the command rather than fixing it). see `TestLegacyCommandKeepsStdoutCleanWithoutAPIKey`.
