...
- deliberate exception to the above (CON-683): `api/query.go` no longer prints the missing-api-key error to stdout, it returns the typed `no_credentials` sentinel; stdout for the legacy commands it backs (`create/get/remove/start/stop pod`, `model`) is otherwise unchanged. `model` errors reach the json sink in `Execute`; the legacy pod commands still `cobra.CheckErr` so theirs stay plaintext on stderr. see `TestLegacyCommandKeepsStdoutCleanWithoutAPIKey`.
